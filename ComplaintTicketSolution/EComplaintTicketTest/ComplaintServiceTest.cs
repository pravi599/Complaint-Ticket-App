...
namespace ComplaintTicketTest
    ...
    public class ComplaintServiceTest
    {
        ...
        [Test]
        public void GetTrackingByComplaintId_ValidComplaintId_ReturnsTrackingDTO()
        {
            // Arrange
            ...
            mockTrackingRepository.Setup(repo => repo.GetAll())
                .Returns(new List<Tracking> { mockTracking });

            // Act
            var result = trackingService.GetTrackingByComplaintId(complaintId);

            // Assert
            Assert.NotNull(result);
            Assert.AreEqual(mockTracking.TrackingId, result.TrackingId);
            Assert.AreEqual(mockTracking.ComplaintId, result.ComplaintId);
            Assert.AreEqual(mockTracking.UpdateDate, result.UpdateDate);
            Assert.AreEqual(mockTracking.Status, result.Status);
        }
        [Test]
        public void GetTrackingByComplaintId_ComplaintNotFound_ThrowsTrackingNotFoundException()
        {
            // Arrange
            var mockTrackingRepository = new Mock<IRepository<int, Tracking>>();
            var mockComplaintRepository = new Mock<IRepository<int, Complaint>>();
            ...
            // Act and Assert
            Assert.Throws<TrackingNotFoundException>(() => trackingService.GetTrackingByComplaintId(complaintId));
        }
